    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        for article_id, orginal_url in zip(id_list, url_list):
            ...
            try:
                ...
                # title = (div.article_head h1.title)
                # content = (div#articlebody p) p 들이 여러개임
                # img = (figure.img_center)
                # --- 나머지 정보 추출 ---
                # 'data["newsTitle"]'이 아닌 상세 페이지에서 추출하거나, 안전한 기본값 사용
                article_title = soup.select_one("div.article_head h1.title").text.strip() if soup.select_one(
                    "div.article_head h1.title") else None
